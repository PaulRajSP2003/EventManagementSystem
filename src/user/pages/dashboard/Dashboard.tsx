...
const Dashboard: React.FC = () => {
  ...
  const [eventId] = useState<number>(2014);
  const [showLiveModal, setShowLiveModal] = useState(false);

  // Enhanced prevCounts to include all values for smooth animations
  ...
  useEffect(() => {
    ...
    const handleNotification = (notification: any) => {
      if (notification.dashboard && data) {
        setPrevCounts({
          students: data.dashboard.attendance.students.present,
          studentAbsent: data.dashboard.attendance.students.absent,
          studentRegistered: data.dashboard.attendance.students.registered,
          leaders: data.dashboard.attendance.leaders.present,
          leaderAbsent: data.dashboard.attendance.leaders.absent,
          leaderRegistered: data.dashboard.attendance.leaders.registered,
          total: data.dashboard.attendance.students.present + data.dashboard.attendance.leaders.present
        });
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-sm text-slate-500">Fetching latest camp data</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-50">
          ...
          <div className="flex items-center gap-6">
            <div>
              ...
              <p className="text-xs text-slate-500 hidden sm:block">
                {eventInfo.eventName} • Day {eventInfo.currentDay} of {eventInfo.duration}
              </p>
            </div>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        {/* Event Info Cards - Always Visible */}
                      ...
                      <div className="p-6">
                        ...
                        >
                          <p className="text-xs text-slate-500 flex items-center justify-center gap-1">
                            <Users className="w-3 h-3" /> Total Participants
                          </p>
                          <p className="text-xl font-bold bg-gradient-to-r from-blue-600 to-pink-600 bg-clip-text text-transparent">
                            <RollingNumber value={(genderDistribution?.total?.male || 0) + (genderDistribution?.total?.female || 0)} />
                          </p>
                        </motion.div>
                      </div>
                    </motion.div>
