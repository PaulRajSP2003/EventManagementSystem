...
const ReplacementStudentDetail = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-50 pb-32">
      <StickyHeader title="Replacement Student Detail">
        {hasAccess() && isDirty && (
          <button
            onClick={handleReset}
            disabled={isSaving}
      ...
      <div className="max-w-6xl mx-auto px-4 mt-6">
        {permissionLoading ? (
        ...
        ) : !isAdmin || permissionError || accessDenied ? (
          ...
          <div className="py-20 text-center text-slate-500">Student not found</div>
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                ...
                <div className="px-6 pb-6 text-center -mt-12 relative">
                  ...
                  <p className="text-slate-500 text-sm mt-1">
                    ID: #{student.id} • {student.age} Years
                  </p>
                  <div className="flex justify-center gap-2 mt-4">
                    <span
                      className={`px-3 py-1 rounded-full text-xs font-bold uppercase ${
                        status === "Present" ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"
                      }`}
                    >
                      {status}
                    </span>
            ...
            <div className="lg:col-span-2 space-y-6">
              <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 capitalize">
                  <InfoItem icon={FiUser} label="Parent Name" value={student.parentName} color="text-blue-500" />
                  <InfoItem
                    icon={FiMapPin}
                    label="Place / City"
                    ...
                    color="text-red-500"
                  />
                  <InfoItem icon={FaChurch} label="Home Church" value={student.churchName} color="text-amber-500" />
                  <InfoItem
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 mt-4">
                    <div
                      className={`flex flex-col gap-2 p-3 rounded-xl border ${
                        !canEditGroup ? "bg-gray-100 opacity-60" : "bg-slate-50"
                      }`}
                    >
                      ...
                      {canEditGroup ? (
                        groupStructure ? (
                          <AvailableFollowingGroupList
                            ...
                            mainGroup={`${student.gender?.toLowerCase() === "male" ? "M" : "F"}${student.mentor_age_group ?? ""}`}
                        ...
                        <div className="flex items-center justify-between">
                          <label
                            className={`text-[10px] font-bold uppercase tracking-wider ${
                              selectedRoomId === "-1" ? "text-red-600" : "text-slate-500"
                            }`}
                          >
                            Room
                          </label>
        ...
        <div className="bg-white/90 backdrop-blur-xl border border-slate-200 shadow-[0_8px_30px_rgb(0,0,0,0.08)] rounded-2xl p-3 flex items-center justify-between ring-1 ring-black/5">
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-[10px] text-slate-500 font-medium mt-0.5">Allocation modified</span>
            </div>
          </div>
          <button
            onClick={handleSave}
