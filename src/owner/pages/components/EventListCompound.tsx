...
interface EventListCompoundProps {
  ...
  loading: boolean;
  error: string;
  onSelectEvent?: (id: number | undefined) => void;
...
const EventListCompound: React.FC<EventListCompoundProps> = ({
  ...
  // Initialize selected event using .id
  useEffect(() => {
    if (selectedEventId !== undefined) {
      const initialEvent = events.find(e => e.id === selectedEventId) || null;
  ...
  const filteredEvents = events.filter(event => {
    // Exclude events that already have an admin assigned
    if (assignedEventIds.includes(event.id)) {
      return false;
    }

    const matchesName = (event.eventName || '').toLowerCase().includes(searchTerm.toLowerCase());
    const matchesId = idFilter === '' || event.id?.toString().includes(idFilter);
              ...
              filteredEvents.map((event) => (
                <div
                  ...
                  className={`px-3 py-2.5 hover:bg-indigo-50 cursor-pointer transition-colors border-b border-slate-100 last:border-0 flex items-center gap-3 ${
                    selectedEventId === event.id ? 'bg-indigo-50 border-l-4 border-l-indigo-600' : ''
                  }`}
